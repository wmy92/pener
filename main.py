...
def filterchild(spanmaps,mapping,k):
    ...
    for label in mapping:
        ...
        if len(mapping[label]) > 0:
            ...
            mapping[label] = [i[0] for i in mapping[label] if i[1] > 0.01]            
...
# 重写上面一部分获取细粒度标签
def supportsetpredllm(model,reader,targetlabel,support,tokenizer):
    filterdata = reader.obtaindatawithtype(support,targetlabel)
    info = datainfo(dataset=filterdata,target_classes=targetlabel,pred=True,typing=True)
    reader.setinfo(info)
    typingdata = reader.read('file')
    ...
    for data in typingdata:
        # y = model.forward_on_instance(data)
        generated=getResponse(targetlabel, data, data['span'])
        spans = data['span'].as_tensor(data['span'].get_padding_lengths())
        generated = generated.strip()
        result.append(typingdecoder(generated,spans))
    return result  

def getResponse(targetlabel, data, span_label):
    result=""
    try:
        ...
        completion = client.chat.completions.create(
            ...
            messages=[
                ...
                {'role': 'user', 'content': f'''
                                            Generate fine-grained labels for entities based on the given annotation
                                            data and coarse-grained categories (e.g. : {targetlabel}). The
                                            fine-grained labels should be based on the entity data and provide a
                                            more specific categorization.
                                            for example: 
                                            Input: France's Jhon on official visit to Greece Sep 15. The entities and their categories are: {span_label}
                                            Output: France is republic, democracy, country, state, sovereign state. Juppe is person, human, politician. Greece is country, sovereign state, state.
                                            
                 
                                            Input: {data} The entities and their categories are: {span_label}'''}
                ]
        )
        ...
        with open(recordfile,'w') as f2:
            for i in range(instancenum):
                ...
                if mode == 'prompt':
                    ...
                    if os.path.exists(modelfile + '/support_predsllm_' + str(i) + '.json'):
                        supportresultllm = []
                        with open(modelfile +'/support_predsllm_' + str(i) + '.json') as f:
                            for j in f:
                                supportresultllm.append(json.loads(j))
                    else:
                        supportresultllm = supportsetpredllm(model,testreader,target_classes,support,tokenizer)
                        with open(modelfile + '/support_predsllm_' + str(i) + '.json','w') as f:
                            for j in supportresultllm:
                                f.write(json.dumps(j)+'\n')
                    supportresult+=supportresultllm
...
if __name__ == "__main__":
    ...
    parser.add_argument("-sdnet", "--sdnet", default=True)
    parser.add_argument("-span", "--span", default=True)  
