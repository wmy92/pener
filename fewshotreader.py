    ...
    def entity2text(self,entitys,labelmap=None):
        ...
        for entity in entitys:
            ...
            if self.target_classes is None or type in self.target_classes:
                ...
                generated.append( entity['text'] + ' is a' + ' ' + type + ' entity.') 
        return generated
        ...
        if typing:
            ...
            for label in instances:
                ...
                for finelabel in labeltoken:
                    if len(labelid) + cum + min(len(finelabelids) - 1, 0) + len(finelabel[0]) <= 15:  # 多个标签平分
                        cum += len(finelabel[0])
                ...
                prefix = prefix + labelid
            prefix.append(32098)
#             prefix=prefix+self.tokenizer.encode(" Extract named entities from the sentence: ",add_special_tokens=False)
    ...
    def getinstance(self,data,pred=False,labels=None,typing=False,index=0):
        ...
        if self.mode == 'prompt' or typing:
            if typing:
                ...
                text=text+" Extract named entities from the sentence."
                inputid = self.tokenizer.encode(text,max_length=511-len(prefix),truncation=True)
                inputid = prefix + inputid
            a=self.tokenizer.decode(inputid)
        else:
...
class spanFSNERreader(DatasetReader):
    def __init__(self, file=None,pretrainedfile=None, mode='prompt', lazy=False) -> None:  # mapping=None, 
        super().__init__(lazy)
        self.tokenizer = AutoTokenizer.from_pretrained(pretrainedfile)
        # self.mapping = mapping
        self.mode = mode
    ...
    def prefix_generator(self):
        ...
        what are entities:
        '''
        textPrompt="what are entities:"
        prefix = self.tokenizer.encode(textPrompt,add_special_tokens=False)
        #         prefix.append(32098)  #???
        return prefix

    def getinstance(self,data,pred=False,index=0):
        text = ' '.join(data['tokens'])
        # labelmap = copy.deepcopy(self.mapping)
        entities = data['entity']
        spans = [entity['text'] for entity in entities]
        generated = self.entity2text(spans)
        generated = ', '.join(generated)
        prefix = self.prefix_generator()
...
#             field['span'] = MetadataField([[self.tokenizer.decode(self.tokenizer.encode(i['text'],add_special_tokens=False)),i['type']] for i in data['entity']])
    ...
    def sampleOneEpoch(self,dataset,pred=False):
        results = []
        for i in range(len(dataset)):           
            if pred:
                results.append(self.getinstance(dataset[i],pred=True,index=i))
            else:
                results.append(self.getinstance(dataset[i],pred=False,index=i))
                random.shuffle(results)
        return results
    ...
    def _read(self,file):
        ...
        if self.info.pred:
            # epochdata = self.sampleOneEpoch(dataset,typing=self.info.typing,labels=self.info.labels,pred=True)
            epochdata = self.sampleOneEpoch(dataset,pred=True)
            for data in epochdata:
                yield data
        else:
#             while True:
            # print("1**\n")
            epochdata = self.sampleOneEpoch(dataset,pred=False)
            # print("2**\n")
            # epochdata = self.sampleOneEpoch(dataset,typing=self.info.typing,labels=self.info.labels,pred=False)
            for data in epochdata:
                # print("3**\n")
